...
// Fast/snappy spring for the shrink (idle → active).
const PILL_SPRING = { type: "spring" as const, stiffness: 500, damping: 22, mass: 0.8 };
// Slower, more deliberate spring for the expand (active → success) — the
// expansion's overshoot is clipped by the container, so the visible motion
// looks faster than the shrink. Heavier mass + tighter damping evens it out.
...
/**
 * Variant — same as EmailCaptureMorph but the *container* width is also driven
 * by Motion's spring (instead of CSS ease-grow), so the outer shell and the
 * inner pill move as one synchronized piece in every transition.
 */
export function EmailCaptureMorphV2({ onSuccess }: { onSuccess?: () => void } = {}) {
  ...
  const fittedW = Math.max(
    FITTED_MIN_W,
    registeredW + FITTED_ICON + FITTED_GAP + FITTED_PAD_X * 2
  );
  ...
  // Use the slower spring whenever we're animating into the success state.
    ...
    >
      <motion.div
        ...
        style={{
          ...
          height: H,
          borderRadius: H,
          backgroundColor: "var(--waitlist-bg)",
          border: `1px solid ${focused ? "rgba(0,153,255,0.28)" : isActive ? "var(--waitlist-border)" : "transparent"}`,
